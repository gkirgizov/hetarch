...
void TrikMspI2c::send(const std::vector<uint8_t> &data)
{
	if (data.size() == 3) {
...
int TrikMspI2c::read(const std::vector<uint8_t> &data)
{
	if (data.size() == 2) {
...
bool TrikMspI2c::connect(const std::string &devicePath, int deviceId)
{
	mDeviceFileDescriptor = open(devicePath.c_str(), O_RDWR);
	if (mDeviceFileDescriptor < 0) {
		std::cerr << "Failed to open I2C device file " << devicePath;
		return false;
	}

	if (ioctl(mDeviceFileDescriptor, I2C_SLAVE, deviceId)) {
		std::cerr << "ioctl(" << mDeviceFileDescriptor << ", I2C_SLAVE, " << deviceId << ") failed ";
		return false;
	}
