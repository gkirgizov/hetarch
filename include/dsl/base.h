#ifndef HETARCH_DSL_BASE_H
#define HETARCH_DSL_BASE_H 1
...
struct ENonGeneric {
    ...
    virtual std::string ptrreg(IRDumper &dumper) const { throw "unimplemented"; }
    virtual std::string valreg(IRDumper &dumper) const { throw "unimplemented"; }
    virtual void toIR(IRDumper &dumper) const { throw "unimplemented"; }
};
...
//RValue<tr> operator oper (RValueBase<t> &&b) \
    //    { return RValueFromE<tr>(new EBin<t, tr>(std::forward<RValueBase<t> >(*this), std::forward<RValueBase<t> >(b), openc)); } \
...
template<typename T> struct RValueNumBase: public RValueBase<T> {
    ...
    RValue<TR> Cast() &&{
        ...
        if (sizeof_safe<T>() > sizeof_safe<TR>()) {
            x = __E_LLVM_IR_CONV_trunc;
        }
        else if (sizeof_safe<T>() < sizeof_safe<TR>()) {
            x = std::is_signed<T>() ? __E_LLVM_IR_CONV_sext : __E_LLVM_IR_CONV_zext;
        }
        else {
            // TODO incomplete typecheck
            x = __E_LLVM_IR_CONV__to_pointer;
        }
        ENonGeneric *eold = this->e;
        this->e = nullptr;
        return RValueFromE<TR>(new EIRConv<T, TR>(eold, x));
    }
...
template<> struct LValue<T>: public RValue<T> { \
    ...
    RValue<T*> operator &() const { return RValueFromE<T*>(new EGetAddr<T>(*this)); } \
    RValue<T*> GetAddr() const { return RValueFromE<T*>(new EGetAddr<T>(*this)); } \
    LValue<T> operator++() { return Assign(*this + RValue<T>(1)); } \
    LValue(ENonGeneric *_e, bool _unused): RValue<T>(_e, false) {} \
};
...
template<typename T> struct EParam: public E<T> {
    ...
    VALREG_OVERRIDE(dumper)        { return GetVarname(); }
    std::string GetVarname() const { return std::string("%") + name; }
};

std::string ebin_valreg(IRDumper &dumper, Operations op, std::string lhsr, std::string rhsr, std::string rett);
template<typename T, typename TR> struct EBin: public E<TR> {
    ...
    EBin(RValueBase<T> &&a, const RValueBase<T> &b, Operations _op):
            lhs(std::forward<RValueBase<T> >(a)), rhs(b), op(_op) {}
    EBin(RValueBase<T> &&a, RValueBase<T> &&b, Operations _op):
            lhs(std::forward<RValueBase<T> >(a)), rhs(std::forward<RValueBase<T> >(b)), op(_op) {}
    VALREG_OVERRIDE(dumper) {
        auto lhsr = lhs.valreg(dumper);
        return ebin_valreg(dumper, op, lhsr, rhs.valreg(dumper), IRTypename<T>());
    }
...
template<typename T, typename TR> struct EIRConv: E<TR> {
    ...
    EIRConv(ENonGeneric *a, __E_LLVM_IR_CONV _conv): from(RValueFromE<T>(a)), conv(_conv) {}
    VALREG_OVERRIDE(dumper) {
...
public:
    ...
    Function(const std::string &_name, bool _isInline, FunctionParams &&_params, RValue<Ret> &&_body):
            FunctionBase<AddrT>(_name, _isInline), params(_params), body(std::forward<RValue<Ret> >(_body)) {}
